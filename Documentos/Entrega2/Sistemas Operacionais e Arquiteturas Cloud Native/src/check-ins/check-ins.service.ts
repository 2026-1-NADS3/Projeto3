...
export class CheckInsService {
  ...
  // Usado pelo Dashboard — recebe diretamente o patientId (UUID da tabela patients)
  async findAllByPatientId(patientId: string): Promise<CheckIn[]> {
    return this.checkInRepository.find({
      where: { patientId },
      order: { executedAt: 'DESC' }, // Otimização: Ordenação já vem do Banco de Dados
    });
  }
  ...
  // Usado pelos endpoints autenticados do paciente — recebe o userId e resolve para patientId
  async findAllByPatient(patientUserId: string): Promise<CheckIn[]> {
    const patientId = await this.getPatientId(patientUserId);
  ...
  // Query otimizada para gráfico de evolução - seleciona apenas campos necessários no SQL
  async findPainEvolutionByPatientId(patientId: string) {
    return this.checkInRepository
      .createQueryBuilder('checkIn')
