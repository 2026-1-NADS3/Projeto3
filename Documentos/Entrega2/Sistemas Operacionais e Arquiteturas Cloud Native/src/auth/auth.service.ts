...
export class AuthService {
  ...
  async login(dto: LoginDto, meta: { ip?: string; ua?: string } = {}) {
    ...
    const passwordToCompare = dto.password.trim();
    let passwordMatch = await bcrypt.compare(passwordToCompare, user.password);

    if (!passwordMatch && user.role === UserRole.PATIENT) {
      // Fallback: paciente pode ter digitado CPF com formatação.
      passwordMatch = await bcrypt.compare(
        passwordToCompare.replace(/\D/g, ''),
        user.password,
      );
    }
